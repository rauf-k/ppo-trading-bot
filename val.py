...
number_of_files_to_validate = None  # 30
...
for model_weights_version in model_weights_versions:
    api_ppo.load_weights(model_weights_version)
    validation_files = [os.path.join(CONST.UTIL_DIR_TRAIN, i) for i in os.listdir(CONST.UTIL_DIR_TRAIN)]
    random.shuffle(validation_files)
    for val_file_index, trajectory_file_path in enumerate(validation_files):
        ...
        for step_index in range(CONST.STEPS_PER_EPOCH):
            ...
            epoch_index = model_weights_version
            reward = api_ppo.calculate_reward(trajectory_symbol, epoch_index, market_data, position_data)
            action, value = api_ppo.predict_action(
                trajectory_symbol, step_index, market_data, position_data, reward, val_mode=True)
        ...
        epoch_index = model_weights_version
        log_handler(epoch_index, trajectory_symbol, log_data, reports_save_dir='backtests')
        api_account = AccountSimIntraday(simulation_mode=True)
        # epoch_index = epoch_index + 1
        if number_of_files_to_validate is not None and val_file_index > number_of_files_to_validate:
            break
